...
│ ├── sprites/
│ └── sounds/
├── src/ # Game modules (player, map, camera)
└── libs/ # Optional Lua libraries (HUMP, STI, etc.)
```
---
...
HUMP – Camera, timers, and utilities.

STI (Simple Tiled Implementation) – Tilemap loader (optional for Tiled maps).
...
Optimize for performance and portability (PC → mobile).
